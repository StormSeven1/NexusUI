...
import {
  ...
  TRACK_EVAL_TIME_REFRESH_INTERVAL_MS,
  useTrackEvaluationStore,
} from "@/stores/track-evaluation-store";

/** 页面启动后连接航迹评估 WS；开始/结束时间每分钟滚动；勾选「开启自动分析」时每 3 分钟自动查询 */
export function useTrackEvalAutoQuery() {
  ...
  const refreshTimeWindow = useTrackEvaluationStore((s) => s.refreshTimeWindow);
