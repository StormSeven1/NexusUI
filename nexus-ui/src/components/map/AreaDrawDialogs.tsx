      ...
      <div className="flex max-h-[min(85vh,640px)] w-full max-w-3xl overflow-hidden rounded-lg border border-white/10 bg-[#141418] shadow-xl">
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button
            ...
            disabled={shape !== "route" && groupMode === "existing" && groups.length === 0}
            onClick={startDraw}
          >
            开始绘制
          </button>
...
export function AreaDrawSaveDialog() {
  ...
  useEffect(() => {
    if (pending) setName(defaultName);
  }, [pending, defaultName]);

  if (!pending) return null;

  const { session, geometry } = pending;
  const shapeLabel = SHAPE_OPTIONS.find((s) => s.id === pending.shape)?.label ?? pending.shape;
