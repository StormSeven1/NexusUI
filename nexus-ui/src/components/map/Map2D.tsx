...
import {
  DbAreaDrawMaplibre,
  type DbAreaDrawCompletePayload,
 ...
 *
 * 2) 用户注册区域/航线（`Custombackend WS DbAreas + db-area-store`）
 *    这套正式业务对象不再挂进 `lyr-zones`，而是走独立的 `LYR_DB_AREAS` 图层。
 */
...
export function Map2D() {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      const init = async () => {
        ...
        /* WS 业务限制区与测量多边形仍由 `PolygonDrawMaplibre` 负责；用户注册区/航线另走 `DbAreaDrawMaplibre`。 */
        ...
        setDbAreasGeoJSON(
          ...
          buildDbAreasFeatureCollection(
            ...
            (row) => {
              const key = dbAreaVisibilityKey(row.group_id, row.area_id);
              return useDbAreaStore.getState().areaVisibility[key] !== false;
            },
          ),
        );
        const dbAreaDraw = new DbAreaDrawMaplibre(map, {
          onComplete: (payload: DbAreaDrawCompletePayload) => {
            const session = useAreaDrawStore.getState().session;
            if (!session) {
              useMapMeasureUi.getState().setActiveDrawTool(null);
              return;
            }
            useAreaDrawStore.getState().setPendingSave({ ...payload, session });
          },
          onCancel: () => {
        ...
        dbAreaDraw.init(HIGHLIGHT_LAYER);
        dbAreaDrawRef.current = dbAreaDraw;
        /*
        ...
        const measureHandlers: Map2DMeasureHandlers = {
          ...
          startAreaDraw(session) {
            ...
            angle.deactivate();
            poly.deactivate();
            dbAreaDraw.activate(session.shape);
        ...
        map.on("click", (e) => {
          ...
          const areaHit = map.queryRenderedFeatures(e.point, { layers: [...DB_AREAS_LAYER_IDS] }).find((f) => {
            const groupId = Number(f.properties?.groupId);
            const areaId = Number(f.properties?.areaId);
            return Number.isFinite(groupId) && Number.isFinite(areaId);
          });
          if (areaHit) {
            const groupId = Number(areaHit.properties?.groupId);
            const areaId = Number(areaHit.properties?.areaId);
            const areaType = Number(areaHit.properties?.areaType);
            const areaName = String(areaHit.properties?.name ?? "未命名区域");
  ...
  /* 注册区域/航线：统一以 `db-area-store.rows` 为事实源，渲染独立 `LYR_DB_AREAS`。 */
  useEffect(() => {
    const updateDbAreas = () => {
      const map = mapRef.current;
      if (!map?.isStyleLoaded()) return;
      const { rows, areaVisibility } = useDbAreaStore.getState();
      const data = buildDbAreasFeatureCollection(rows, (row) => {
        const key = dbAreaVisibilityKey(row.group_id, row.area_id);
        return areaVisibility[key] !== false;
      });
      setDbAreasGeoJSON(map, data);
      applyLayerPanelVisibilityFromStore(map, useAppStore.getState());
      for (const layerId of DB_AREAS_LAYER_IDS) {
        try {
          if (!map.getLayer(layerId)) continue;
          map.setLayoutProperty(
            layerId,
            "visibility",
            (useAppStore.getState().layerVisibility[LYR_DB_AREAS] ?? true) ? "visible" : "none",
          );
        } catch { /* skip */ }
      }
      map.triggerRepaint();
    ...
    updateDbAreas();
    const unsub = useDbAreaStore.subscribe(updateDbAreas);
    return unsub;
  }, []);
