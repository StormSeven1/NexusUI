 ...
 */
function speedMetersPerSecondForVector(speed: number): number {
  if (!(typeof speed === "number" && Number.isFinite(speed) && speed > 0)) return VECTOR_FALLBACK_SPEED_MS;
  if (speed <= 200) return speed * KNOTS_TO_METERS_PER_SEC;
  return speed;
}
 ...
 * - **距离**：节→m/s × 秒；若仍短于阈值则拉长到 `VECTOR_MIN_GROUND_METERS` 便于辨认。
...
export function velocityVectorEndLngLat(t: Track, vectorLengthSeconds: number): [number, number] | null {
  ...
  const spdMs = speedMetersPerSecondForVector(
    typeof t.speed === "number" && Number.isFinite(t.speed) ? t.speed : NaN,
  );
  let dist = spdMs * sec;
  if (dist > 0 && dist < VECTOR_MIN_GROUND_METERS) dist = VECTOR_MIN_GROUND_METERS;
