...
/** 从查证 HTTP/SSE 载荷解析 unique_id（Body 字段优先，否则按 trackID 查内存航迹） */
export function resolveVerifyUniqueId(
  payload: TaskStatusChatPayload,
  tracks: readonly Track[],
): string | null {
  const fromBody = normUniqueId(
    (payload as TaskStatusChatPayload & { uniqueId?: number; unique_id?: number }).uniqueId ??
      (payload as TaskStatusChatPayload & { unique_id?: number }).unique_id,
  );
  if (fromBody) return fromBody;

  const track = findTrackForTaskStatusVerify(payload.trackID, tracks);
  if (track) {
